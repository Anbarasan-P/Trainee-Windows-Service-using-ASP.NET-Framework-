...
namespace TraineeService
{
    ...
    {
        //string connectionString = @"Server=localhost\SQLEXPRESS;Database=TraineeDB;Integrated Security=True";
        string connectionString = @"Server=localhost\SQLEXPRESS;Database=TraineeDB;User Id=TraineeUser1;Password=YourPassword123!";
        ...
        public void BackupTraineeTable()
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    System.IO.File.AppendAllText(@"D:\TraineeServiceDebugLogsss.txt",
    $"Connected to DB: {connection.Database} | Login: {connection.WorkstationId} | Time: {DateTime.Now}\r\n");
...
//                        SqlCommand checkCommand = new SqlCommand("SELECT COUNT(*) FROM TraineesBackup WHERE TraineeID=@TraineeID", connection);
//                        checkCommand.Parameters.AddWithValue("@TraineeID", traineeId);
//                        int rowExists = (int)checkCommand.ExecuteScalar();
